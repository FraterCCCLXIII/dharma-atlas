/**
 * Original Turnstile-style scrolling world spinner
 * (CodePen bluebie/JjdoaLG — `.world` + `world-scroll`).
 */
export function EarthSpinner({ className = "" }: { className?: string }) {
  return (
    <>
      {/* Sprite lives outside the clipped circle (same as the original pen). */}
      <svg
        aria-hidden
        className="earth-spinner__defs"
        version="1.1"
        xmlns="http://www.w3.org/2000/svg"
      >
        <defs>
          <symbol id="earth-icon-world" viewBox="0 0 216 100">
            <title>world</title>
            <g fillRule="nonzero">
          ...
          <symbol id="earth-icon-repeated-world" viewBox="0 0 432 100">
            <use href="#earth-icon-world" x="0" />
            <use href="#earth-icon-world" x="189" />
          </symbol>
        </defs>
      </svg>

      <span className={`earth-spinner ${className}`}>
        <svg>
          <use href="#earth-icon-repeated-world" />
        </svg>
      </span>
    </>
  );
